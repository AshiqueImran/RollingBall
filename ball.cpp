...
void updateObjects(int value){
	...
	if(_move_o1 > 5.0){
			_move_o1 = -4.5;
			_obj_color_r = getRamdomNum(-1.0, 1.0);
			_obj_color_g = getRamdomNum(-2.0, 2.0);
			_obj_color_b = getRamdomNum(-2.0, 2.0);
			_obj_pos_y = getRamdomNum(-0.5, -2.0);
	}
